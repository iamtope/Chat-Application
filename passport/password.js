...
crypto.randomBytes(config.crypto.randomSize, function(err, salt){
    if (err)
    return cb(err, null);
    crypto.pbkdf2(password, salt.toString('base64'), config.
    crypto.workFactor, config.crypto.keylen, 'sha256', function(err, key){
   ...
   salt, work, cb){
   crypto.pbkdf2(password, salt, work, config.crypto.keylen, 'sha256',
   function(err, key){
